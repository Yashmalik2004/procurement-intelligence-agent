...
class SupplierScore(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    score_id: Mapped[str] = mapped_column(String, unique=True, nullable=True)  # correlation key returned by POST
    supplier_name: Mapped[str] = mapped_column(String, nullable=False)
    category: Mapped[str] = mapped_column(String, nullable=True)              # stored for UI display
